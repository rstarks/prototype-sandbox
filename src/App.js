...
class RecipePage extends Component {
  ...
  // update to include pagination displaying step based on the currentPage

  render() {
    if (this.state.page+1 == this.state.steps.length) {
      return (
        <button className='btn btn-primary float-right' type='button' onClick={this.nextPage}>Finish</button>
      );
    } else {
      return (
        <div style={{...defaultStyle,display:'inlineblock',width: "25%"}}>
          ...
          <button className='btn btn-primary float-right' type='button' onClick={this.nextPage}>Continue to Step {this.state.page + 2}</button>
        </div>
      );
    }
  }
